...
subject: workflow
lang: en
date: 2023-04-12      # Scrivener placeholder
verbose: false                 # pandoc verbosity
...
format:
  ...
  pdf:
    ...
    mathfont: Menlo  # !!! Change to a font on your system
    linkcolor: Mahogany
...
# CiteTools Integration

[]{#scriv27}

By Bernardo Vasconcelos (see this)

This is a short summary of changes to Ian's excellect [Quarto template for Scrivener](http://forum.literatureandlatte.com/t/scrivener-quarto-a-technical-academic-publishing-workflow/129769?u=bernardo_vasconcelos) to integrate the [Cite Tools](http://bcdavasconcelos.github.io/citetools/) Quarto Extension. The extension introduces bibliography features that previously had been confined to `BibTeX` to Pandoc's Citeproc environment, keeping the output consistent across most formats (`LaTeX`, `DOCX`, `HTML`, and so on).
...
[2. New]{.smallcaps} **Section Type** & **Custom Metadata Field** can be used to override the depth of the current binder item placing the desired number of hashes in the corresponding metadata field (`Depth`) with no space at the end, as in `###`*,* and then selecting the  `Depth .. Header & Text` section type.

[3. New]{.smallcaps} **Styles** provide support for the [cite field](https://github.com/bcdavasconcelos/citefield) Lua Filter, which can be used to cite arbitrary fields of the references.
...
: Simple example of how we are using styles to create the correct markup {#tbl-cite-field-example}


[4. Added]{.smallcaps} the prefix `[]{#scriv\<$linkID>}` to some **Section Layouts**. 

This allows us to create links to [text sections that do not have a Header element](#scriv26).   
...
Later, the [@DA]{.title} was edited by [@DABiehl1896]{.editor} in [@DABiehl1896]{.issued} [repr. in [@DATheiler]{.translator}'s [@DATheiler]{.issued} translation]; and by [@DAForster]{.editor} in [@DAForster]{.issued} [repr. in [@DACorcilius]{.translator}' [@DACorcilius]{.issued} translation]; and by [@DARoss]{.editor} in [@DARoss]{.issued}.
```
...
# Primary Sources {.unnumbered}

[]{#scriv33}

---
bibliography_multibib33: "https://raw.githubusercontent.com/bcdavasconcelos/citetools/main/refs/primary.json"
---
...
# Secondary Sources {.unnumbered}

[]{#scriv34}

---
bibliography_multibib34: "https://raw.githubusercontent.com/bcdavasconcelos/citetools/main/refs/secondary.json"
---
...
---
bibliography_multibib35: "https://raw.githubusercontent.com/bcdavasconcelos/scrivomatic/master/refs/workflow.bib"
---
